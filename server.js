...
app.get('/buckets/:bucketName/files/:fileName', async (req, res) => {
    try {
        const result = await client.getFileInfo(req.params.bucketName, req.params.fileName);
        res.json({ success: true, data: result });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
...
app.post('/buckets/:bucketName/files', upload, async (req, res) => {
    try {
        ...
        if (uploadedFile) {
            ...
            result = await client.uploadFile(req.params.bucketName, req.body.filePath);
        } else {
...
app.get('/buckets/:bucketName/files/:fileName/download', async (req, res) => {
    try {
        ...
        // Download the file
        await client.downloadFile(req.params.bucketName, req.params.fileName, downloadDir);
